...
export function deriveState(sp) {
  ...
  for (const w of weeks) {
    ...
    if (s) { next = { title: s.name, url: s.url, week: w.week, score: s.score, outOf: s.outOf }; break; }
  }
  ...
  return {
    ...
    rows: weeks.map((w) => ({ week: w.week, done: w.done, items: w.items.map((i) => ({ name: i.name, url: i.url, complete: i.complete, score: i.score, outOf: i.outOf })) })),
  };
}
