...
async function edgeGet(path) {
  const b = base();
  if (!b) return null;
  const r = await fetch(`${b.url}/functions/v1/${path}`, { headers: { apikey: b.key, "content-type": "application/json" } });
  if (!r.ok) throw new Error(`${path.split("?")[0]} ${r.status}`);
  return r.json();
}

export async function getCurriculumState(courseId, studentId) {
  const d = await edgeGet(`phase-config?action=curriculum_state&course_id=${encodeURIComponent(courseId)}&user_id=${encodeURIComponent(studentId)}`);
  return d && d.configured ? d : { configured: false };
}

// The whole journey (all phases), from the same student_progress the PhaseJourney stepper
// uses. Trimmed to what the agent shows: each phase's name, where the student is, and how
// many they've passed vs need.
export async function getPhaseJourney(courseId, studentId) {
  const d = await edgeGet(`phase-config?action=student_progress&course_id=${encodeURIComponent(courseId)}&user_id=${encodeURIComponent(studentId)}`);
  if (!d || d.configured === false) return { configured: false };
  const phases = Array.isArray(d.phases) ? d.phases : [];
  const currentIndex = typeof d.currentPhaseIndex === "number" ? d.currentPhaseIndex : phases.findIndex((p) => p.status === "active");
  return {
    configured: true,
    journeyComplete: !!d.journeyComplete,
    currentPhaseName: phases[currentIndex]?.name || null,
    phases: phases.map((p, i) => ({
      name: p.name,
      status: p.status === "done" ? "done" : i === currentIndex ? "active" : p.status || "upcoming",
...
export function deriveState(cs) {
  ...
  for (const w of due) {
    ...
    if (item) {
      next = { title: shortName(item.name), url: item.url, week: w.weekNumber, module: w.moduleName, score: item.earnedScore, outOf: item.requiredScore };
      break;
    }
  }
  ...
  return {
    ...
    // Full week list (used by the explicit "show me everything" card, any view). Each item
    // carries its score/required so the card shows "35 / 100 · need 80%".
    rows: due.map((w) => ({
      week: w.weekNumber,
      done: rowDone(w),
