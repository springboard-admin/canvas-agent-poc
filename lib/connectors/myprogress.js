...
import { getModules } from "../canvas.js";
...
async function fetch(ctx) {
  ...
  return { sp, status, journey: derivePhases(sp), unavailable: status.state === "unknown", courseId: ctx.courseId, studentId: ctx.studentId };
}

// Canvas item link for a week, used only when My Progress has no URL for it.
async function canvasWeekUrl(courseId, studentId, week) {
  const modules = await getModules(courseId, studentId);
  const mod = modules.find((m) => {
    const hit = String(m.name || "").match(/week\s*(\d+)/i);
    return hit && parseInt(hit[1], 10) === Number(week);
  });
  const item = (mod?.items || []).find((it) => it.url);
...
const tools = [
  {
    def: { name: "get_progress", description: "Current progress in the student's ACTIVE phase (Curriculum, Exam Prep, ExCPT Exam, Externship…): state, which phase, gate, cutoff, counts, score, and any phase guidance. For 'how am I doing' / 'what do I do next' — answer in WORDS, don't dump the list.", input_schema: { type: "object", properties: {} } },
    run: (_i, d) => (d.unavailable ? { unavailable: true } : { state: d.status.state, label: d.status.label, phaseName: d.status.phaseName, phaseIndex: d.status.phaseIndex, totalPhases: d.status.totalPhases, journeyComplete: d.status.journeyComplete, guidance: d.status.guidance, units: d.status.units, gate: d.status.gate, passPercent: d.status.passPercent, dueCount: d.status.dueCount, doneCount: d.status.doneCount, score: d.status.score, scoreBand: d.status.scoreBand, next: d.status.next ? { title: d.status.next.title, week: d.status.next.week } : null }),
  },
  {
  ...
  },
  {
    ...
    run: async ({ week }, d, cards) => {
      let url = weekUrl(d.sp, week);
      if (!url) url = await canvasWeekUrl(d.courseId, d.studentId, week).catch(() => null);
      if (!url) return { shown: false };
      cards.push({ kind: "open", title: `Week ${week}`, url, week, cta: "Open in Canvas →" });
      return { shown: true };
    },
  },
  {
  ...
  {
    def: { name: "open_practice", description: "Render a card linking the student to EXTRA PRACTICE (the RxReps practice-exam tool). Use when they say they feel under-prepared, nervous about the exam, or want more practice.", input_schema: { type: "object", properties: {} } },
    run: (_i, _d, cards) => {
      const url = env("PRACTICE_LTI_URL", "https://rxreps.lovable.app/app/index.html");
      if (!url) return { shown: false };
      cards.push({ kind: "open", title: "Extra practice", url, cta: "Start practice →" });
      return { shown: true };
    },
  },
];
