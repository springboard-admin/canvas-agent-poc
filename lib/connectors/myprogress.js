...
const tools = [
  {
    ...
    run: (_i, d) => (d.unavailable ? { unavailable: true } : { state: d.status.state, label: d.status.label, phaseName: d.status.phaseName, phaseIndex: d.status.phaseIndex, totalPhases: d.status.totalPhases, journeyComplete: d.status.journeyComplete, guidance: d.status.guidance, gate: d.status.gate, passPercent: d.status.passPercent, dueCount: d.status.dueCount, doneCount: d.status.doneCount, score: d.status.score, scoreBand: d.status.scoreBand, next: d.status.next ? { title: d.status.next.title, week: d.status.next.week } : null }),
  },
  {
