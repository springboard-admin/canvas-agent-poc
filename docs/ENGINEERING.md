...
| **Live schedule** | only when `get_live_schedule` runs | In-process stub, same payload for every course. No DB call. **Backlog:** replace `livesessions` fetch with a DB read by `courseId`. |
